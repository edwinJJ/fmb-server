...
public interface IMcsFMachineStorageQueryManager extends JpaRepository<McsFMachineStorage, String>  {
	...
	@Query(value = "SELECT "
			...
			+ "		,(SELECT COUNT(*) FROM MCS_T_TRANSPORT_JOB_INF WHERE DESTMACHINENAME = ?1) AS reservedCount"
			...
			+ "		(SELECT "
			+ "			COUNT(CASE WHEN (carrier.EMPTYTYPE = ?2) THEN 1 ELSE NULL END) as empty"
			+ "			,COUNT(CASE WHEN (carrier.EMPTYTYPE = ?2) THEN 1 ELSE NULL END) as emptyEmpty"
			+ "			,COUNT(CASE WHEN (carrier.EMPTYTYPE = ?2) THEN 1 ELSE NULL END) as full"
			+ "			,COUNT(CASE WHEN (carrier.ID LIKE = ?3) THEN 1 ELSE NULL END) as unkCarrier "
	...
	public Object getStockerPopupData(@Param("id") String id, @Param("empty") String empty, @Param("unknownRule") String unknownRule);
}
