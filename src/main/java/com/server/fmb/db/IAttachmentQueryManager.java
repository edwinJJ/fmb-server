...
//public interface IAttachmentQueryManager extends JpaRepository<Attachments, UUID> {
	...
	@Query(value = "SELECT u.* FROM attachments u ORDER BY u.name DESC", nativeQuery = true)
...
//	public List<Attachments> getAttachments(@Param("start") int start, @Param("end") int end);
	...
	@Query(value = "SELECT u.* FROM attachments u WHERE u.category = ?1  ORDER BY u.name DESC", nativeQuery = true)
